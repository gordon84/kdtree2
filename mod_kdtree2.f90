...
! =======
!
! A self-contained Fortran implementation of Matthew B. Kennel's k-d tree
! for finding near neighbors in multidimensional Euclidean data. The original
! repository also contained C++ sources and generated documentation; this
! compact edition intentionally contains only this module and its example test.
!
! The input layout is data(1:D,1:N): dimensions first and observations second.
! Search distances are squared Euclidean distances. The input data must have
! the TARGET attribute and must remain alive until kdtree2_destroy is called.
!
! Citation
! --------
...
! neighbors in a multi-dimensional Euclidean space," arXiv:physics/0408067.
!
! Quick example
! -------------
...
!   type(typeKdtree2Result) :: results(N_NEAREST)
!
!   tree => kdtree2_create(data, sort=.true., rearrange=.true.)
!   call kdtree2_n_nearest(tree, query, N_NEAREST, results)
!   call kdtree2_destroy(tree)
!
! Tree traversal starts at tree%root. A leaf has neither left nor right
! associated; its points are tree%ind(node%l:node%u). See test_kdtree2.f90
! for complete search and recursive traversal examples.
!
! Compile and run
! ---------------
...
module mod_kdtree2_priority_queue
  ...
  ! is the priority.
  !
  type typeKdtree2Result
      ! a pair of distances, indexes
      real(rt) :: dis ! Squared Euclidean distance from the query.
      integer :: idx  ! One-based index of the matching input point.
  end type typeKdtree2Result
  !
  ...
  type typePriorityQueue
      ...
      ! to match.
      integer :: heap_size = 0                         ! Number of active heap elements.
      type(typeKdtree2Result), pointer :: elems(:)     ! Caller-owned result storage.
  end type typePriorityQueue

  public :: typeKdtree2Result

  public :: typePriorityQueue
  public :: pq_create
...
contains
  ...
  function pq_create(results_in) result(res)
    ...
    !    pq => pq_create(x,k)
    !
    type(typeKdtree2Result), target:: results_in(:) 
    type(typePriorityQueue) :: res
    !
    !
    integer :: nalloc
...
!  end function compare_priority
  ...
  subroutine heapify(a,i_in)
    ...
    !
    type(typePriorityQueue),pointer   :: a
    integer, intent(in) :: i_in
    !
    integer :: i, l, r, largest

    real(rt)    :: pri_i, pri_l, pri_r, pri_largest


    type(typeKdtree2Result) :: temp
...
bigloop:  do
  ...
  subroutine pq_max(a,e) 
    ...
    !
    type(typePriorityQueue),pointer :: a
    type(typeKdtree2Result),intent(out)  :: e
  ...
  real(rt) function pq_replace_max(a,dis,idx) 
    !
...
!    type(typeKdtree2Result), intent(in) :: e
    ...
    real(rt)    :: prichild, prichildp1

    type(typeKdtree2Result) :: etmp
...
end module mod_kdtree2_priority_queue
...
module mod_kdtree2
  ...
  public :: rt
  public :: typeInterval, typeKdtree2, typeKdtree2Result, typeTreeNode
  ...
  type :: typeInterval
      real(rt) :: lower ! Lower coordinate bound.
      real(rt) :: upper ! Upper coordinate bound.
  end type typeInterval

  type :: typeTreeNode
      ! an internal tree node
      integer :: cut_dim                             ! Dimension used to split this node; zero for a leaf.
      real(rt) :: cut_val                            ! Coordinate value used to divide the children.
      real(rt) :: cut_val_left                       ! Upper coordinate bound of the left child.
      real(rt) :: cut_val_right                      ! Lower coordinate bound of the right child.
      integer :: l                                   ! First position in the tree's permuted index array.
      integer :: u                                   ! Last position in the tree's permuted index array.
      type(typeTreeNode), pointer :: left => null()  ! Left child, or null for a leaf.
      type(typeTreeNode), pointer :: right => null() ! Right child, or null for a leaf.
      type(typeInterval), pointer :: box(:) => null() ! Bounding interval in each dimension.


  end type typeTreeNode

  type :: typeKdtree2
      ! Global information about the tree, one per tree
      integer :: dimen=0, n=0
      ! dimensionality and total # of points
      real(rt), pointer :: the_data(:,:) => null()
      ...
      real(rt), pointer :: rearranged_data(:,:) => null()
      ...
      type (typeTreeNode), pointer :: root => null()
      ! root pointer of the tree
  end type typeKdtree2


  type :: typeTreeSearchRecord
      !
      ...
      logical           :: overflow
      real(rt), pointer :: qv(:)  ! query vector
      type(typeKdtree2Result), pointer :: results(:) ! results
      type(typePriorityQueue) :: pq
      real(rt), pointer :: data(:,:)  ! temp pointer to data
      integer, pointer      :: ind(:)     ! temp pointer to indexes
  end type typeTreeSearchRecord

  private
  ! everything else is private.

  type(typeTreeSearchRecord), save, target :: sr   ! A GLOBAL VARIABLE for search
...
contains
  ...
  function kdtree2_create(input_data,dim,sort,rearrange) result (mr)
    ...
    type(typeKdtree2), pointer :: mr
    integer, intent(in), optional      :: dim
    logical, intent(in), optional      :: sort
    logical, intent(in), optional      :: rearrange
    ! ..
    ! .. Array Arguments ..
    real(rt), target :: input_data(:,:)
    !
    integer :: i
    ! ..
    allocate (mr)
    ...
    subroutine build_tree(tp)
      type(typeKdtree2), pointer :: tp
      ! ..
      integer :: j
      type(typeTreeNode), pointer :: dummy => null()
      ! ..
    ...
    recursive function build_tree_for_range(tp,l,u,parent) result (res)
      ! .. Function Return Cut_value ..
      type (typeTreeNode), pointer :: res
      ! ..
      ! .. Structure Arguments ..
      type(typeKdtree2), pointer :: tp
      type (typeTreeNode),pointer           :: parent
      ! ..
      ! .. Scalar Arguments ..
      ...
      if ((u-l)<=BUCKET_SIZE) then
         ...
         if (.false.) then
            ...
            if (.true.) then
               ...
               average = sum(tp%the_data(c,tp%ind(l:u))) / real(u-l+1,rt)
            else
...
!         res%cut_val = tp%the_data(c,tp%ind(m))
   ...
   subroutine spread_in_coordinate(tp,c,l,u,interv) 
      ...
      ! .. Structure Arguments ..
      type(typeKdtree2), pointer :: tp
      type(typeInterval), intent(out) :: interv
      ! ..
      ! .. Scalar Arguments ..
      integer, intent (In) :: c, l, u
      ! ..
      ! .. Local Scalars ..
      real(rt) :: last, lmax, lmin, t, smin,smax
      integer :: i, ulocal
      ! ..
      ! .. Local Arrays ..
      real(rt), pointer :: v(:,:)
  ...
  subroutine kdtree2_n_nearest(tp,qv,nn,results)
    ...
    type(typeKdtree2), pointer      :: tp
    real(rt), target, intent (In)    :: qv(:)
    integer, intent (In)         :: nn
    type(typeKdtree2Result), target :: results(:)
...
!    deallocate(sr%pqp)
  ...
  subroutine kdtree2_n_nearest_around_point(tp,idxin,correltime,nn,results)
    ...
    sr%ballsize = huge(1.0)       ! the largest real(rt) number
    sr%centeridx = idxin
  ...
  recursive subroutine search(node)
    ...
    !
    type (typeTreeNode), pointer          :: node
    ! ..
    type(typeTreeNode),pointer            :: ncloser, nfarther
    !
    integer                            :: cut_dim, i
    ! ..
    real(rt)                               :: qval, dis
...
!          dis=extra**2
          if (dis <= ballsize) then
             ...
             do i=1,sr%dimen
                if (i .ne. cut_dim) then
                   dis = dis + distanceFromBound(qv(i),box(i)%lower,box(i)%upper)
                   if (dis > ballsize) then
                      return
                   endif
                endif
  ...
  ! Return the squared distance between a coordinate and a closed interval.
  real(rt) function distanceFromBound(x, lowerBound, upperBound) result(distanceSquared)
    real(rt), intent(in) :: x, lowerBound, upperBound

    if (x > upperBound) then
       distanceSquared = (x - upperBound)**2
    elseif (x < lowerBound) then
       distanceSquared = (lowerBound - x)**2
    else
       distanceSquared = 0.0_rt
    endif
  ...
  subroutine process_terminal_node(node)
    ...
    real(rt), pointer          :: qv(:)
    integer, pointer       :: ind(:)
    real(rt), pointer          :: data(:,:)
    !
    integer                :: dimen, i, indexofi, k, centeridx, correltime
    real(rt)                   :: ballsize, sd, newpri
    logical                :: rearrange
    type(typePriorityQueue), pointer      :: pqp 
    !
  ...
  subroutine kdtree2_n_nearest_brute_force(tp,qv,nn,results) 
    ...
    ! does.
    type(typeKdtree2), pointer :: tp
    real(rt), intent (In)       :: qv(:)
    integer, intent (In)    :: nn
    type(typeKdtree2Result)    :: results(:) 

    integer :: i, j, k
    real(rt), allocatable :: all_distances(:)
    ! ..
  ...
  subroutine heapsort_struct(a,n)
    ...
    !
    integer,intent(in)                 :: n
    type(typeKdtree2Result),intent(inout) :: a(:)

    !
    !
    type(typeKdtree2Result) :: value ! temporary value
